...
export default function EditProductScreen ({ navigation, route }) {
  ...
  const [initialProductValues, setInitialProductValues] = useState({ name: null, description: null, price: null, order: null, productCategoryId: null, scheduleId: null, availability: null, image: null })
  ...
  useEffect(() => {
    async function fetchRestaurantSchedules () {
      try {
        const fetchedRestaurantSchedules = await getRestaurantSchedules(route.params.restaurantId)
        console.log(fetchedRestaurantSchedules)
        const fetchRestaurantSchedulesReshaped = fetchedRestaurantSchedules.map((e) => {
          return {
            label: e.startTime + ' - ' + e.endTime,
            value: e.id
          }
        })
        setRestaurantSchedules([{ label: 'Not scheduled', value: null }, ...fetchRestaurantSchedulesReshaped])
      } catch (error) {
        showMessage({
          ...
          <View style={{ alignItems: 'center' }}>
            <View style={{ width: '60%' }}>
              ...
              <DropDownPicker
                ...
                setItems={setRestaurantSchedules}
