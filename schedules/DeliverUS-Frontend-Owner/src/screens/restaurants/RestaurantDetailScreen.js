...
export default function RestaurantDetailScreen ({ navigation, route }) {
  ...
  const scheduleColor = (schedule) => {
    return schedule?.startTime ? GlobalStyles.brandGreenTap : GlobalStyles.brandPrimary
  }
    ...
    return (
      ...
      >
        ...
        <View style={styles.row}>
          <MaterialCommunityIcons name='timetable' size={18} style={{ color: scheduleColor(item.schedule) }}/>
          <TextRegular numberOfLines={2} textStyle={[styles.price, { color: scheduleColor(item.schedule) }]}>
        ...
        {!item.availability &&
         ...
         <View style={styles.actionButtonsContainer}>
          <Pressable
            onPress={() => navigation.navigate('EditProductScreen', { id: item.id, restaurantId: route.params.id })
            }
            style={({ pressed }) => [
              {
                backgroundColor: pressed
